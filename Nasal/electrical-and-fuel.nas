...
#
# Modified By G ROBIN
# Modified by M. Kraus in Avril, 2012 
...
update_electrical = func {
    ...
    running = getprop("sim/model/h21/state") or 0;
    dt = time - last_time;
    last_time = time;

    update_virtual_bus( dt );
    ################################################## FUEL CONTROL ##########################################
    if(running) update_fuel( dt );
    ##########################################################################################################
...
######################### Hack to give the informations to the copilot #############################
var tank_control = func {

  var tank_1 = props.globals.getNode("/consumables/fuel/tank[0]/mk-level-gal_us");
  var tank_2 = props.globals.getNode("/consumables/fuel/tank[1]/mk-level-gal_us");

  var total = tank_1.getValue() + tank_2.getValue();
  setprop("/consumables/fuel/mk-total-fuel-gals", total);

  settimer(tank_control, 1);
