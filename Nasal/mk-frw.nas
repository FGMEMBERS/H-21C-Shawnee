...
var frw_show_time_on_screen = func{
  ...
  if (hours > 0){
    screen.log.write(sprintf("%3dh %02dmin %02dsec", hours, minutes, seconds), 1, 1, 0.0);
  }else{
    screen.log.write(sprintf("%02dmin %02dsec", minutes, seconds), 1, 1, 0.0);
  }
}
...
var frw_control = func {
  ...
  if(frw_mode.getBoolValue()){
    # set here the altitude-agl-ft minus the altitude-agl-ft on ground 
    var frw_agl  = getprop("/position/altitude-agl-ft") - 8.2;
